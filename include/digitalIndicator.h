...
void indicatorInit(void){
    ...
    digitalWrite(DIA_REQ_PIN, LOW); // Non-request state
...
float indicatorReadMM(){
    ...
    for(i=0; i<13; i++){
        ...
        for(j=0; j<4;j++){
            ...
            while(digitalRead(DIA_CLK_PIN) == LOW){}    // Wait for clk to go high
            while(digitalRead(DIA_CLK_PIN) == HIGH){}   // Wait for clk to go low

            // Read the current bit and write to in_byte in place j (restores rightmost LSB)
   ...
   if(units==1) value = value*25.4;     // Converts in reading to mm
   
   // Find and apply sign of reading
   if(sign==8) value = -value;  // Negative sign case
